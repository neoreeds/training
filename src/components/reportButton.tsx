...
export class Report extends React.Component<ReportProps, any> {
    constructor(props, context) {
        ...
        this.writeSheetData = this.writeSheetData.bind(this);
        this.tryCatch = this.tryCatch.bind(this);
    }

    click = async () => {
        this.tryCatch(this.createReport);
    }
    ...
    createReport = async () => {     
        await Excel.run(async (context) => {
            ...
            try {
                await this.writeSheetData(sheet);
                sheet.activate();
                await context.sync();
            }
            catch (error) {
    ...
    writeSheetData = async (sheet: Excel.Worksheet) => {
    ...
    render() {
        return (
            <Button className='nr-action' buttonType={ButtonType.hero} icon='ChevronRight' onClick={this.click}>Créer Report</Button>
        );
    };
};
